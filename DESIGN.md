...
2. **发丝线代替阴影**：卡片静止时只有 1px 半透明边线，没有阴影；阴影只留给悬浮的草稿卡。激活态只把边线加深一级并加一层极淡阴影——类型色不参与选中态。
...
类型色（内置，灰调）：agree `#6FA287` · disagree `#C96C6C` · question `#D4A24C` · important `#6A8FCF` · note `#9AA0A6`。自定义类型由 `main.ts injectTypeStyles()` 写入 `.ilc-card-<id> { --ilc-accent }`。
